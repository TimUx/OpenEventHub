...
- Admin Import settings **coverage** as expandable region hierarchy (like Regions); selecting a parent checks all child regions
...
- Admin Events: **Lokalität** (Venue.name) vs **Ort (Region)** vs **Adresse** klar getrennt; Tabellenspalte zeigt `Lokalität · Ort`
- Compound-POI-Heuristik erkennt `Schlosskirche`/`*kirche` für Siedlungs-Extract (`settlementQueryFromLabel`)
- Venue-Persistenz kürzt Adresse auf Straßenzeile (`Paradeplatz`) und Lokalität ohne Siedlungs-Suffix (`Schlosskirche`); Toubiz liefert nur Straße; DEV-Repair normalisiert bestehende Venues

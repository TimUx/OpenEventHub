#!/usr/bin/env bash
# Reset categories to the curated rural DE catalog and remap event links.
# Safe for DEV: does not delete events — only category rows / links.
set -euo pipefail
...
docker run --rm \
  ...
  node:22-bookworm \
  bash -lc "npm run db:generate && npx tsx packages/database/prisma/reset-categories.ts"

echo "Category catalog reset successfully."

...
> Language: English · [Deutsch (primary)](../ROADMAP.md)
...
(see `CHANGELOG.md`, latest **v0.27.0**):

- **v0.27.0:** Admin import coverage as expandable region tree; parent selects all child regions
- **v0.26.1:** Venue locality normalize only with region hint or compound *kirche* (no Museum/Gasthaus stripping)
- **v0.26.0:** Admin venue vs place (region); venue address/name normalization; compound *kirche* heuristics
...
- Event density heatmap at `/heatmap` (Apache ECharts calendar heatmap; year→month→week→weekend→day) with category/region filters; classic calendar unchanged
...
- Dedicated `toubiz` plugin **1.1.0** (`mein.toubiz.de` / `<toubiz-widget>`): `eventDates` + event detail for address/description; EMS category (`sourceCategories`) before LLM
- Admin Events: bulk status/delete via batch endpoints (no 429 rate limit)
